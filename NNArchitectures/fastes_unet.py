...
def EntryFlow(input, num_output):
  a = Convolution2D(num_output/32, 1, activation='relu', padding='same')(input)
  a = Convolution2D(num_output/32, 6, strides=(4,4), activation='relu')(a)
  a = Convolution2D(num_output, 1, activation='relu', padding='same')(a)
...
#
  ...
  #1198 -> 299
  conv56 = Convolution2D(6,6, strides=(4,4),activation='relu', padding='same')(conv112)

  #299 -> 74
  pool28 = EntryFlow(conv56,  filters*2)
  conv28 = MiddleFlow(pool28, filters*2) 
  
  #74 -> 18
  pool14 = EntryFlow(conv28,  filters*4)
  ...
  up28 = concatenate([ZeroPadding2D(1)(UpSampling2D(size=(4, 4))(up14)), conv28], axis=-1)
  up28 = Convolution2D(filters*2, (1, 1), activation='relu')(up28)


  up56 = concatenate([ZeroPadding2D(2)(UpSampling2D(size=(4, 4))(up28)), conv56], axis=-1)
  up56 = Convolution2D(filters, (1, 1), activation='relu')(up56)

  up112 = concatenate([(UpSampling2D(size=(4, 4))(up56)), ZeroPadding2D(1)(conv112)], axis=-1)
